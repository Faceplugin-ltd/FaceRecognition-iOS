...
public enum FaceJSON {
    ...
    private static func parseVideoWorkerFaces(_ faces: [[String: Any]]) -> [VideoWorkerFace] {
        faces.compactMap { face in
            ...
            let pose = face["facePose"] as? [String: Any]
            let (liveness, livenessLabel) = parseWorkerLiveness(face)
    ...
    private static func parseWorkerLiveness(_ face: [String: Any]) -> (Float, String?) {
        var label = blankToNil(face["livenessLabel"] as? String)
            ?? blankToNil(face["liveness_label"] as? String)
        var score = doubleValue(face["liveness"])
        let attrs = face["attributes"] as? [String: Any]
        let live = (attrs?["Liveness2D"] as? [String: Any]) ?? (attrs?["liveness"] as? [String: Any])
        if let live {
            if label == nil {
                label = blankToNil(live["value"] as? String)
            }
            if score == nil {
                score = doubleValue(live["confidence"]) ?? doubleValue(live["value"])
            }
        }
        return (Float(score ?? 0), label)
    }
    ...
    public static func toDetectedFace(_ face: VideoWorkerFace) -> DetectedFace {
        var attributes: [String: FaceAttribute] = [:]
        if let label = face.livenessLabel, !label.isEmpty {
            attributes["Liveness2D"] = FaceAttribute(
                value: label,
                confidence: String(format: "%.3f", face.liveness)
            )
        } else if face.liveness > 0.001 {
            attributes["Liveness2D"] = FaceAttribute(
                value: String(format: "%.3f", face.liveness),
                confidence: String(format: "%.3f", face.liveness)
            )
        }
        return DetectedFace(
